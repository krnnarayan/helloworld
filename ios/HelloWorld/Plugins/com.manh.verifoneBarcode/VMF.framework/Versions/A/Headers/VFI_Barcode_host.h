...
typedef struct {
    long long length;                        // total length of the host packet (max value 2048 bytes)
    long long sequence;                      // host command sequence number (serial number assigned by host)
...
typedef struct {
    long long length;						//total length of the response packet
    long long sequence;						//response packet sequence number
	long long status;						//last received packet process status, 0 - success in processing whole packet, 1 - failure
